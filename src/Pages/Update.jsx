...
const Update = (props) => {
  ...
  const EncodingDesc = (desc) => {
    ...
    const flushList = () => {
      if (listBuffer.length > 0) {
        ...
        else if (listType === "ol") result.push(`<ol>${listBuffer.join("")}</ol>`);
        listBuffer = [];
        listType = null;
      }
    };
  ...
  const parseDescLine = (line) => {
    ...
    if (trimmed.includes("${{\u00A0") && trimmed.includes("\u00A0}}")) {
      ...
      const innerContent = codedLink.replace("${{\u00A0", "").replace("\u00A0}}", "").trim();
  ...
  // Fetch todo data
  useEffect(() => {
    firebase.getDataByDoc(`todo/${user.uid}_${sno}`).then((snap) => {
      const data = snap.data();
      reset({
        title: data.title,
        desc: data.decodedDesc,
  ...
  const submit = (e) => {
    ...
    firebase.addData(`todo/${user.uid}_${sno}`, todo);
    navigate("/");
  };
          ...
          <form onSubmit={handleSubmit(submit)} className="form main">
            ...
            <div className="form-floating mb-3">
              ...
              <div className="desc-bottom">
                <i className="fa-solid fa-link desc-btn" onClick={() => setWantLink(true)} title="Add Link"></i>
                <i className="fa-solid fa-list desc-btn" title="Add UnOrdered List Item" onClick={handleOrderList}></i>
                <i className="fa-solid fa-list-ol desc-btn" title="Add Ordered List Item" onClick={handleUnOrderedList}></i>
                <i className="fa-solid fa-heading desc-btn" title="Add heading" onClick={handleAddHeading}></i>
                {!DueDateAdd ? (
                  <i className="fa-solid fa-calendar desc-btn" onClick={() => setDueDateAdd(true)} title="Add due Date"></i>
                ) : (
                  <i className="fa-solid fa-calendar-xmark desc-btn" onClick={() => setDueDateAdd(false)} title="Remove due Date"></i>
                )}
              </div>
            </div>
            ...
            <div className="form-bottoms">
              <Link to="/" className="link">
                <input className="btn btn-outline-primary" type="reset" value={"Cancel"} />
              </Link>
              ...
              <form className="link-model form" onSubmit={handleAddLink}>
                ...
                <div className="form-floating mb-3">
                  <input
                    ...
                    autoComplete="off"
                    value={LinkUrl.split("").length === 0 ? `https://www.${LinkUrl}` : LinkUrl}
